                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                       ...
                       <tr v-for="(item,index) in lista" :key="item.id">
                        ...
                        <td>{{item.id}}</td> 
                          <td><img :src="'http://localhost:8000/nossafrica/storage/app/public/'+item.curso_img" alt="" class="img-responsive img-rounded img-thumbnail" width="120px"></td> 
                           <td>{{item.curso_nome}}</td> 
                          <td>{{item.curso_preco}} kz</td> 
                           <td>{{item.curso_duracao}}</td>
                           <td>{{item.name}}</td>
                           <td>{{item.cat_nome}}</td>
                                      ...
                                      <form v-bind:id="index" v-if="deletar && token" v-bind:action="deletar + item.id" method="post">     
                                          <input type="hidden"   name="_method" value="DELETE">
                                            <input type="hidden" name="_token" v-bind:value="token"> 
                                             <a v-if="alunos" v-bind:href="alunos+item.id" class="btn btn-xs btn-warning"><i class="fa fa-eye"></i>inscritos</a>
          
                                          <a v-if="modulo" v-bind:href="modulo+item.id" class="btn btn-xs btn-success"><i class="fa fa-eye"></i>modulo</a>
