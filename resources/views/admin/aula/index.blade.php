...
<pagina tamanho="12">
    <painel titulo="Lista Aula">
        ...
        <aula
         ...
         v-bind:itens="{{json_encode($listaAula)}}"
...
<modal nome="adicionar" titulo="Aula">
<formulario css="" action="{{route('aula.store')}}" method="post" enctype="multipart/form-data" token="{{ csrf_token() }}">  
  ...
  <select  name="modulo_id" class="form-control">
    <option value="">Seleciona o Modulo</option>	
    @foreach($listaModulos as $listas)
     <option value="{{$listas->id}}">{{$listas->modulo_titulo}}</option>	
    @endforeach						
  </select>
</div>  
</div>
