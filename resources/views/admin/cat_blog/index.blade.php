...
<div class="site">
        ...
        <div class="caixa">
			<h2 class="titulo"><span class="case"><i class="ico duvida"></i>Categorias do blog</span></h2>
        </div>
...
<pagina tamanho="12">
    <painel>
        ...
        <cat_blog
         ...
         v-bind:itens="{{json_encode($listaCatBlog)}}"
...
<formulario css="" action="{{route('cat_blog.store')}}" method="post" enctype="multipart/form-data" token="{{ csrf_token() }}">  
    {{ csrf_field() }}
...
<modal nome="editar" titulo="Categorias">
    <formulario id="formEditar" v-bind:action="'/cat_blog/' + $store.state.item.id" method="put" enctype="multipart/form-data" token="{{ csrf_token() }}">
       {{ csrf_field() }}
